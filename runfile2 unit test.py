...
while user_input != 'x':
    ...
    if user_input_c == user_input_b:
        ...
        while user_input_a != 'x':
            new_flight = Flight_trip(flight_number='', destination='Rome', origin='Geneva', duration='1:00')
            new_aircraft = Aircraft(aircraft_number='',capacity='',age='')
            user_input_Ive_lost_count = input("enter aircraft number: ")
            new_aircraft.create_aircraft_number(user_input_Ive_lost_count)
            new_aircraft.get_aircraft_number()
            user_input_ab = input("enter flight number: ")
            new_flight.create_flight_number(user_input_ab)
            new_flight.get_flight_number()
            print(new_flight.get_flight_number(), new_aircraft.get_aircraft_number())
            print('capacity: ', new_aircraft)
            while user_input_ab is not None:
                new_ticket = Tickets(aircraft_number='',capacity='', age=int(), ticket_price='',
                ...
                print('ticket price: ', new_ticket.get_ticket_price())
                # new_ticket.decrement(user_input_part4), new_aircraft.set_capacity(new_aircraft.get_capacity())
                # print(new_aircraft.get_capacity())
